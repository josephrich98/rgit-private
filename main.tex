...
of a molecular state assayed in the tumour tissue itself. A natural test of this
reading is to swap the imaging modality for digital \emph{histopathology} --- the
H\&E whole-slide images (WSI) of the same TCGA-KIRC tumours --- while holding the
...
over $\wsiNcases$ cases, mapped to Case ID and tumour/normal status through the
...
groups: H\&E \emph{stain} (Ruifrok--Johnston colour-deconvolution optical
...
are averaged over its tumour slides into one case-level vector
...
\paragraph{Tumour vs.\ adjacent-normal.}
If the histology--expression coupling is genuinely tumour-biological rather than a
generic tissue or batch effect, a patient's tumour-derived expression should be
reflected in their \emph{tumour} slides but not in their \emph{adjacent-normal}
...
($n=\wsiNnormal$) against $\wsiRone$ for tumour --- a gap that survives
...
it is not merely the larger tumour-slide count. The molecular signal is read out
...
$\wsiRank$). The histology--genomics coupling is therefore not an artefact of
...
tumour-specific: tumour slides (red) carry the expression signal that
...
All analysis code and Lean~\citep{moura2021lean4} proof scripts for the main mathematical propositions are available at \url{https://github.com/pachterlab/rgit}.
