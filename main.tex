...
inference, including a scale-matched permutation null that prevents
high-dimensional inflation from masquerading as signal. The mathematics is
deliberately classical; the contributions are the recoverability framing, the
honest-rank estimation pipeline, and a cautionary three-cohort study. Applied to
TCGA-KIRC, an NSCLC cohort, and ADNI, the same pipeline returns three different
verdicts: only TCGA-KIRC shows a small ($\hat R^{\mathrm{cv}}\approx 0.1$,
single-direction) association that survives both cross-validation and
demographic/scanner deconfounding, whereas the apparent NSCLC signal is
cohort-specific overfit and the ADNI signal an age/sex artefact --- all three of
which a naive in-sample estimate would report as strong positives. The companion
notebook \texttt{notebooks/radiogenomic\_recoverability.ipynb} implements the full
...
Radiogenomics asks how much of a tumour's --- or an organ's --- molecular state
...
The dominant empirical paradigm trains one predictor per gene or per signature
and reports where imaging ``predicts'' genomics. At the cohort sizes typical of
the field ($n\sim 10^2$) against panels of $10^3$--$10^4$ transcripts, this is a
...
confounding. We argue that the more useful and more answerable question is not
``does imaging predict gene $j$?'' but \emph{how many genomic directions are
image-identifiable at all, and how well} --- a low-dimensional, estimable
quantity with a hard ceiling set by the biology--imaging channel rather than by
the size of the gene panel.

To make this precise we adopt the simplest model with a non-trivial answer: a
linear--Gaussian channel from genomics $g\in\R^p$ to imaging $x\in\R^d$. Under
it every quantity of interest is closed-form --- the Bayes-optimal estimator
$\E[g\mid x]$, the posterior covariance, the mutual information $I(g;x)$, and a
per-direction \emph{recoverability score} $R(v)\in[0,1]$ --- and the directions
of maximal recoverability coincide exactly with canonical correlation analysis
(CCA) between the two modalities, with $R(a_i)=\rho_i^2$, the squared canonical
correlation. The model thus turns a vague ``fundamental limit'' intuition into a
concrete, estimable spectrum $\rho_1^2\ge\rho_2^2\ge\cdots$ and an associated set
of genomic directions that can be read back onto genes and pathways.

\paragraph{What is and is not new.}
We want to be explicit about provenance, because the mathematics here is
deliberately classical and the contribution lies elsewhere. The closed forms in
Sections~\ref{sec:joint}--\ref{sec:eigenproblem} --- the joint Gaussian law
(Prop.~\ref{prop:joint}), the Gaussian posterior (Prop.~\ref{prop:posterior}),
the determinant/SNR formula for $I(G;X)$ (Prop.~\ref{prop:mi}), the
canonical-coordinate diagonalization (Prop.~\ref{prop:canonical}), and the
equivalence of the recoverability eigenproblem with CCA (Prop.~\ref{prop:eig})
--- are all standard consequences of Gaussian conditioning, Hotelling's
CCA~\citep{hotelling1936cca}, and the Gaussian mutual-information identity
(\citealp{cover2006elements}; see also probabilistic CCA,
\citealp{bach2005pcca}). We claim them as \emph{assembly and framing}, not as new
theorems, and we attribute each in place. Our actual contributions are three:
\begin{enumerate}
\item \textbf{The recoverability framing.} We define a per-direction
recoverability score $R(v)$ and an image-identifiable genomic subspace, show
$R(a_i)=\rho_i^2$, and give the rank limit (Prop.~\ref{prop:rank}) that caps the
identifiable dimension at a property of the channel, not the panel. We also
extend the construction to binary mutation data via a Gaussian-copula/probit
prior (Section~\ref{sec:bernoulli}), and make precise the \emph{signed} direction
of the Gaussian misspecification error (Eq.~\eqref{eq:mi-lower-bound}), which to
our knowledge is not stated elsewhere in this setting.
\item \textbf{A scale-matched honest-rank pipeline.} CCA in the $p\gtrsim n$
regime inflates canonical correlations toward $1$ by pure geometry. We give an
estimator that pairs symmetric dimension reduction with a cross-validated
recoverability whose permutation null is built \emph{on the same
(cross-validated) scale} as the statistic it calibrates
(Section~\ref{sec:estimation-honest}); naively thresholding a CV-deflated
statistic against an in-sample-inflated null forces the estimated rank to zero
regardless of true signal.
\item \textbf{A cross-cohort empirical study, read cautiously.} We apply the same
pipeline to three cohorts (TCGA-KIRC, an NSCLC radiogenomic set, and ADNI) and
obtain three different verdicts. The headline is sobering: of the three, only
one (KIRC) shows a small association that survives both honest validation and
demographic/scanner deconfounding; the NSCLC signal is cohort-specific overfit
and the ADNI signal is an age/sex artefact. The value of the framework is
precisely that it separates these cases, where a naive in-sample estimate would
report all three as strong positives.
\end{enumerate}

\paragraph{A note on tone.}
This is as much a cautionary methods paper as a positive-result paper. Read
literally, our strongest empirical finding is that confound-robust radiogenomic
signal is scarce and low-dimensional even in favourable cohorts. We have tried
to state magnitudes conservatively, to report cross-validated and
permutation-calibrated numbers rather than in-sample ones throughout, and to
flag where a result rests on a single configuration. The reader should come away
with calibrated skepticism, not with the impression that imaging broadly recovers
genomics.
...
many tumour types~\citep{gevaert2012nsclc,jamshidi2015ccrcc,karlo2014ccrcc,shinagare2015ccrcc,udayakumar2024radiogenomic,bi2019ai,hosny2018ai},
...
less data than proving it is not an artefact of the high-dimensional geometry,
...
rank $k=6$ (the sixth correlation near zero). In-sample $\hat R_i$ (grey) is
...
Table~\ref{tab:assumptions}. The deviation is largest in the small tumour
cohorts (a,~b) and mildest in ADNI (c), the cohort with the most favourable
...
non-normality; the joint remains mildly heavy-tailed (worst in the small tumour
...
favourable direction: because canonical correlation captures only linear
...
The favourable direction is not heuristic; the copula pins it down. Write
...
genuinely narrow, not an artefact of either bias.
...
demographic or scanner artefact.}
...
entirely a demographic artefact:
...
\subsection{Demographic deconfounding: the channel is an age/sex artefact}
...
noise, a demographic artefact. The model's value here is precisely that it converts an eye-catching
...
mirror of the tumour cohorts: the binding constraint for ADNI is not sample
...
analysis to canonical correlation analysis between the modalities. The
mathematics is classical (Section~\ref{sec:intro}); the contributions are the
recoverability framing and rank limit, the scale-matched cross-validated /
permutation pipeline that keeps high-dimensional inflation from masquerading as
signal, and a three-cohort study that puts the pipeline to work.
...
``signal'' is an age/sex artefact that collapses under deconfounding. A naive
...
for the tumour cohorts. (v)~Two cohorts are small ($n=190$, $n=129$), at which
