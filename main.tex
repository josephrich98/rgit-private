...
spanning two tumour cohorts and a non-oncologic brain cohort
...
The two tumour cohorts pair a localized lesion with a matched tissue genomic
...
log-scale intensities; we therefore disable the count-oriented CPM and second
$\log_{1+}$ steps and reduce to the top $2000$ HVGs before the Gaussian-copula
transform. Intersecting on subject ID retains $n = 702$ subjects with
...
components. The honest spectrum is the strongest of the three cohorts:
$5$-fold cross-validation gives a leading recoverability
$\hat R_1^{\mathrm{cv}} = 0.37$ and a clearly resolved second direction
$\hat R_2^{\mathrm{cv}} = 0.12$, with the remainder collapsing to $\approx 0$
...
$p < 0.05$, leading $\hat R^{\mathrm{cv}}$ median $0.37$ (range $0.30$--$0.43$),
and $\hat k_{\mathrm{eff}}$ median $2$ (range $2$--$3$).

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{\adnifig{cv_recoverability.pdf}}
\caption{Recoverability spectrum on ADNI ($n=702$, FastSurfer
morphometry $+$ PyRadiomics, copula marginals). The honest cross-validated
spectrum (blue) resolves \emph{two} genomic directions
($\hat R_1^{\mathrm{cv}} = 0.37$, $\hat R_2^{\mathrm{cv}} = 0.12$) above the
permutation null before collapsing --- the strongest and highest-rank channel
of the three cohorts (compare Figs.~\ref{fig:kirc-spectrum},
\ref{fig:nsclc-spectrum}).}
\label{fig:adni-spectrum}
\end{figure}
...
$2.38$ bits, sitting \emph{above} the permutation band (null mean $1.75$ bits,
$95\%$ upper limit $1.82$ bits): unlike KIRC, where the observed value merely
...
only through cross-validation and permutation --- the same lesson as the tumour
cohorts, but here the honest signal is large enough that even a few hundred
subjects suffice to register it cleanly.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{\adnifig{sample_complexity_sweep.pdf}}
\caption{Sample-complexity sweep on ADNI. The honest $\hat R_1^{\mathrm{cv}}$
...
in-sample $\hat\rho_1^2$ (grey) hugs the Wachter edge (green) and the
permutation $95\%$ null (red) throughout.}
\label{fig:adni-sweep}
\end{figure}
...
gives a clean separation, sharper than in either tumour cohort:
\begin{center}
\begin{tabular}{lcc}
\toprule
group               & mean AUC (logistic) & mean AUC (RF) \\
\midrule
image-identifiable  & $0.853$ & $0.848$ \\
non-identifiable    & $0.483$ & $0.480$ \\
\bottomrule
\end{tabular}
\end{center}
The image-identifiable transcripts are decodable from brain morphometry at
AUC $\approx 0.85$ while the non-identifiable controls sit at chance, and the
random forest does not exceed the linear ceiling --- exactly the falsifiable
pattern the recoverability score predicts (Fig.~\ref{fig:adni-pergene}).

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{\adnifig{per_gene_auc.pdf}}
\caption{Per-gene predictability on ADNI. Five high- and five
low-recoverability transcripts, two classifiers, $5$-fold CV. The
high-recoverability group reaches AUC $\approx 0.85$; the low-recoverability
group sits at the $0.5$ chance line.}
\label{fig:adni-pergene}
\end{figure}

\paragraph{Demographic mediation.}
A blood-to-brain channel of this strength warrants the same skepticism we
applied to the NSCLC scanner confound. Age and sex are powerful common causes
here: both drive regional brain atrophy (hippocampal and entorhinal volume in
particular) and systemic whole-blood expression, so a substantial part of the
$\hat R_1^{\mathrm{cv}} = 0.37$ channel is very likely age/sex-mediated rather
than a direct molecular readout, and the leading imaging loadings fall on
exactly the medial-temporal regions that track age and disease stage. ADNI
demographic and diagnostic covariates were not available in the processed
matrices used here, so --- unlike the NSCLC batch analysis --- we do not report
a demographic-residualized estimate; we therefore read the ADNI channel as a
genuine but largely demographically mediated association, and flag the
age/sex-deconfounded recoverability as the key open quantity for this cohort.

The contrast across cohorts is the useful object. Three cohorts, the same
model, estimator, and working-dimension rule yield a borderline single
direction ($\hat R^{\mathrm{cv}}\approx 0.1$, $\hat k_{\mathrm{eff}}=1$) in
clear-cell RCC, a solid single direction
($\hat R^{\mathrm{cv}}\approx 0.33$, $\hat k_{\mathrm{eff}}=1$) in NSCLC, and a
strong rank-two channel ($\hat R_1^{\mathrm{cv}}\approx 0.37$,
$\hat k_{\mathrm{eff}}=2$) in ADNI. The recoverable dimension is a property of
the biology--imaging channel (Proposition~\ref{prop:rank}); these three
channels differ in both strength and rank while obeying the same estimation
geometry --- exactly the regime the model is built to quantify.
